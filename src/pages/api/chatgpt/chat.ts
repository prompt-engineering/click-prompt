...
import { decrypt, getLocalUser, secret } from "@/pages/api/chatgpt/user";
import { getAllChats, getUserById, updateChatById } from "@/storage/planetscale";

export const createNewOpenAIApi = (apiKey: string) => {
...
const mockHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  console.log(localUser, "localUserlocalUserlocalUser");
  if (!localUser) {
  ...
  if (req.method === "POST" && req.body) {
    ...
    if (prompt && chatId) {
      ...
      try {
        const response = await localUser.openai.createChatCompletion({
          model: "gpt-3.5-turbo",
          messages: [...chat],
          temperature: 0.5,
          max_tokens: 1024,
