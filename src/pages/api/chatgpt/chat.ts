    ...
    switch (body.action) {
      case "send": {
        ...
        await sendMsgs({
          ...
          msgs: chats.map(it => ({role: it.role, content: it.content, name: it.name} as ChatCompletionRequestMessage)),
...
async function sendMsgs({res, client, conversation_id, msgs, newMsgs}: {res: NextApiResponse, client: OpenAIApi, conversation_id: number,  msgs: ChatCompletionRequestMessage[], newMsgs: ChatCompletionRequestMessage[]}) {
  try {
